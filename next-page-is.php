<?php
/*
 * Code copied from Next Page plugin, which is no longer maintained
 * http://sillybean.net/code/wordpress/next-page/
 * /
*/
...
//	
//	// set defaults
	$is_nextpage_options = array();
	$is_nextpage_options['before_prev_link'] = '<div class="alignleft prev-next-button">';
	$is_nextpage_options['prev_link_text'] = '< Previous';
	$is_nextpage_options['after_prev_link'] = '</div>';
	
	$is_nextpage_options['before_parent_link'] = '<div class="aligncenter">';
	$is_nextpage_options['parent_link_text'] = 'Up one level';
	$is_nextpage_options['after_parent_link'] = '</div>';
	
	$is_nextpage_options['before_next_link'] = '<div class="alignright prev-next-button">';
	$is_nextpage_options['next_link_text'] = 'Next >';
	$is_nextpage_options['after_next_link'] = '</div>';
	
	$is_nextpage_options['exclude'] = '';
	$is_nextpage_options['loop'] = 0;
//	
//	// set new option
...
function is_flatten_page_list($exclude = '') {
...
function is_get_next_link() {
	global $post;
        global $is_nextpage_options;
	$exclude = $is_nextpage_options['exclude'];
	$pagelist = is_flatten_page_list($exclude);
	$current = array_search($post->ID, $pagelist);
	$nextID = $pagelist[$current+1];
	
	if (!isset($nextID)) 
		if ($is_nextpage_options['loop'])
			$nextID = $pagelist[0];
		else 
			return '';
	
	$before_link = stripslashes($is_nextpage_options['before_next_link']);
	$linkurl = get_permalink($nextID);
	$title = get_the_title($nextID);
	$linktext = $is_nextpage_options['next_link_text'];
	if (strpos($linktext, '%title%') !== false) 
		$linktext = str_replace('%title%', $title, $linktext);
	$after_link = stripslashes($is_nextpage_options['after_next_link']);
	
	$link = $before_link . '<a href="' . $linkurl . '" title="' . $title . '">' . $linktext . '</a>' . $after_link;
	return $link;
} 

function is_get_previous_link() {
	global $post;
        global $is_nextpage_options;
	...
	$before_link = stripslashes($is_nextpage_options['before_prev_link']);
	$linkurl = get_permalink($prevID);
	$title = get_the_title($prevID);
	$linktext = $is_nextpage_options['prev_link_text'];
	if (strpos($linktext, '%title%') !== false) 
		$linktext = str_replace('%title%', $title, $linktext);
	$after_link = stripslashes($is_nextpage_options['after_prev_link']);
	
	$link = $before_link . '<a href="' . $linkurl . '" title="' . $title . '">' . $linktext . '</a>' . $after_link;
	return $link;
} 

function is_get_parent_link() {
	global $post;
        global $is_nextpage_options;
        
	$parentID = $post->post_parent;
	
	$exclude = array($is_nextpage_options['exclude']);
	if (in_array($parentID, $exclude)) return false;
	else {
		$before_link = stripslashes($is_nextpage_options['before_parent_link']);
		$linkurl = get_permalink($parentID);
		$title = get_the_title($parentID);
		$linktext = $is_nextpage_options['parent_link_text'];
		if (strpos($linktext, '%title%') !== false) 
			$linktext = str_replace('%title%', $title, $linktext);
		$after_link = stripslashes($is_nextpage_options['after_parent_link']);
...
function is_next_link() {
	echo is_get_next_link();
}
function is_previous_link() {
	echo is_get_previous_link();
}
function is_parent_link() {
	echo is_get_parent_link();
}

// shortcodes
add_shortcode('is_previous', 'is_get_previous_link');
add_shortcode('is_next', 'is_get_next_link');
add_shortcode('is_parent', 'is_get_parent_link');
