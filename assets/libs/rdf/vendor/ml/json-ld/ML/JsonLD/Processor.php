...
class Processor
{
    ...
    {
        $nodeMap = new StdObject();
        $nodeMap->{'-' . JsonLD::DEFAULT_GRAPH} = new StdObject();
        ...
        foreach ($properties as $property => $value) {
            ...
            foreach ($value as $item) {
                ...
                if (in_array($def['@container'], array('@language', '@index'))) {
                    if (false === property_exists($element, $activeprty)) {
                        $element->{$activeprty} = new StdObject();
                    }
     ...
     */
    private function getValueProfile(StdObject $value, $inversectx)
    {
        $valueProfile = array(
            '@container' => '@set',
            'typeLang' => '@type',
    ...
    public function flatten($element)
    {
        ...
        foreach ($nodeMap as $graphName => $graph) {
            if (!isset($defaultGraph->{$graphName})) {
                $defaultGraph->{$graphName} = new StdObject();
     ...
     */
    private function elementToRdf(StdObject $element)
    {
        if (property_exists($element, '@value')) {
    ...
    {
        $graphs = new StdObject();
        $graphs->{JsonLD::DEFAULT_GRAPH} = new StdObject();
        $graphs->{'@usages'} = new StdObject();
        ...
        foreach ($quads as $quad) {
            ...
            if (false === isset($graphs->{$graphName})) {
                $graphs->{$graphName} = new StdObject();
            }
